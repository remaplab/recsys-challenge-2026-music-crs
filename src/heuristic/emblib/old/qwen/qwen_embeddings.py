...
# ── instruction prompts ──────────────────────────────────────────────────────
# Queries are prefixed with one of these; tracks are encoded with "none" (no
# prefix), exactly like a Qwen3 retrieval setup (instructed query vs raw doc).
QWEN_INSTRUCTIONS = {
...
def track_metadata_text(row: dict[str, Any]) -> str:
    """Render a single track's metadata into the text we feed to Qwen3.

    Format (newline-joined, empty fields dropped):
        Title: ...
        Artist: ...
        Album: ...
        Tags: ...
...
def pool_last_token(hidden: Any, attention_mask: Any):
    """Last-token pooling (Qwen3 style), L2-normalized at fp32.

    Works for left-padded batches (real tokens at the end) and right-padded
    batches (compute per-row length). The query and track encoders both set
    padding_side='left', so the fast path (`hidden[:, -1]`) is taken.
    """
    import torch
...
def encode_qwen_texts(
    *,
    ...
    """One-shot encode a list of texts with Qwen3 and last-token pooling.

    Loads the model once, then iterates in batches. Returns an (N, 1024)
    float32, L2-normalized array. Use this for the (large, one-off) track
    tower. For repeated per-batch query encoding use Qwen3NativeFrozenEncoder,
