...
# emblib was reorganized under emblib.old.* — these helpers live there now.
from emblib.old.data.parsing import build_query_text_v2
from emblib.old.data.user_features import load_user_features
from emblib.old.encoders.base import encode_corpus


# Resolve data/models against the REPO ROOT (parents[3]: scripts → src/heuristic
# → src → repo), NOT the cwd — this script runs from the embeddings-package venv
# but reads/writes the repo's real data/ and models/.
_REPO = Path(__file__).resolve().parents[3]
DATA = _REPO / "data" / "talkpl-ai"
OUT_BASE = _REPO / "models" / "query_emb_cache"
USER_CACHE = _REPO / "models" / "user_features_cache"
...
SPLIT_TO_PARQUET[os.environ.get("BLINDB_SPLIT", "blind_b")] = Path(os.environ.get(
    "BLINDB_PARQUET",
    str(DATA.parent / "talkpl-ai" / "TalkPlayData-Challenge-Blind-B" / "data" / "test-00000-of-00001.parquet")))
