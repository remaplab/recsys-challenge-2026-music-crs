...
def build_icm(
    ...
    def _list_block(col: str) -> csr_matrix:
        df = (
            ...
            .with_columns(pl.col(col).cast(pl.Categorical).to_physical().cast(pl.Int32).alias("fi"))
        )
        if df.is_empty():
            return csr_matrix((n, 1), dtype=np.float32)
        r, c = df["track_idx"].to_numpy(), df["fi"].to_numpy()
        return csr_matrix((np.ones(len(r), np.float32), (r, c)), shape=(n, int(c.max()) + 1), dtype=np.float32)
...
def _cbf_fast_gpu(icm_norm: csr_matrix, k: int, batch: int, shrink: float, verbose: bool) -> csr_matrix:
    ...
    sp_T = sp.transpose(0, 1).coalesce()
    ...
    for start in iters:
        ...
        sims = torch.sparse.mm(sub, sp_T).to_dense()
        if shrink:
            sims = sims / (1.0 + shrink)
