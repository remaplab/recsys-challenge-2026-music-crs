"""Two-Tower v2 enhanced recommender — text + multimodal track features.
...
DEFAULT_TRACK_MODALITIES: tuple[str, ...] = (
...
DEFAULT_TRACK_MODALITY_WEIGHTS: dict[str, float] = {
    "metadata-qwen3_embedding_0.6b": 1.0,
    "lyrics-qwen3_embedding_0.6b": 0.75,
    "attributes-qwen3_embedding_0.6b": 0.75,
    "audio-laion_clap": 0.35,
    "image-siglip2": 0.30,
    "cf-bpr": 0.20,
}
...
def _one_l2(x: np.ndarray) -> np.ndarray:
    n = float(np.linalg.norm(x))
    return (x / n).astype(np.float32, copy=False) if n > 0 else x.astype(np.float32, copy=False)


def _list_matrix(values: list, dim: int, imputed: dict[str, np.ndarray] | None = None,
                 ids: list[str] | None = None) -> tuple[np.ndarray, np.ndarray]:
    arr = np.zeros((len(values), dim), np.float32)
    mask = np.zeros(len(values), bool)
    imputed = imputed or {}
    ids = ids or [""] * len(values)
    for i, v in enumerate(values):
        if (v is None or len(v) == 0) and ids[i] in imputed:
            v = imputed[ids[i]]
        ...
        if a.size != dim:
            continue
        arr[i] = a
        mask[i] = bool(np.any(a))
    return arr, mask


def _load_imputed_metadata(path: str | Path | None) -> dict[str, np.ndarray]:
    if not path:
        return {}
    p = _repo_path(path)
    if not p.exists():
        print(f"[TwoTowerV2Enhanced] imputed metadata not found: {p}")
        return {}
    df = pl.read_parquet(p, columns=["track_id", "metadata-qwen3_embedding_0.6b"])
    out = {}
    for tid, vec in zip(df["track_id"].to_list(), df["metadata-qwen3_embedding_0.6b"].to_list()):
        if vec:
            out[tid] = np.asarray(vec, dtype=np.float32)
    return out


def _load_qwen_track_cache(cache_dir: str | Path | None) -> dict[str, np.ndarray]:
    if not cache_dir:
        return {}
    p = _repo_path(cache_dir)
    ids_path = p / "track_ids.npy"
    emb_path = p / "embeddings.npy"
    if not ids_path.exists() or not emb_path.exists():
        print(f"[TwoTowerV2Enhanced] qwen track cache not found: {p}")
        return {}
    ids = [str(x) for x in np.load(ids_path, allow_pickle=True).tolist()]
    emb = np.asarray(np.load(emb_path, mmap_mode="r"), dtype=np.float32)
    emb = _row_l2(emb)
    return {tid: emb[i] for i, tid in enumerate(ids)}


def _load_dense_track_vectors(
    track_ids: list[str],
    embedding_paths: list[str] | None,
    qwen_track_cache_dir: str | None,
    imputed_metadata_embedding_path: str | None,
    track_modalities: list[str] | None,
    use_item_cf: bool,
    item_cf_weight: float,
) -> tuple[dict[str, np.ndarray], dict[str, bool], int]:
    modalities = list(track_modalities or DEFAULT_TRACK_MODALITIES)
    if use_item_cf and "cf-bpr" not in modalities:
        modalities.append("cf-bpr")
    if not embedding_paths and not qwen_track_cache_dir:
        return {}, {}, 0

    id_set = set(track_ids)
    blocks_by_id: dict[str, list[np.ndarray]] = {tid: [] for tid in track_ids}
    valid_by_id: dict[str, bool] = {tid: False for tid in track_ids}
    seen_block_by_id: dict[str, set[str]] = {tid: set() for tid in track_ids}
    imputed = _load_imputed_metadata(imputed_metadata_embedding_path)

    if embedding_paths:
        cols = ["track_id", *modalities]
        for path in embedding_paths:
            p = _repo_path(path)
            if not p.exists():
                continue
            available = pl.read_parquet(p, n_rows=0).columns
            read_cols = [c for c in cols if c in available]
            if "track_id" not in read_cols:
                continue
            frame = pl.read_parquet(p, columns=read_cols)
            tids = frame["track_id"].to_list()
            for col in modalities:
                if col not in frame.columns:
                    continue
                dim = KNOWN_EMBEDDING_DIMS[col]
                repl = imputed if col == "metadata-qwen3_embedding_0.6b" else None
                mat, mask = _list_matrix(frame[col].to_list(), dim, repl, tids)
                mat = _row_l2(mat)
                weight = item_cf_weight if col == "cf-bpr" else DEFAULT_TRACK_MODALITY_WEIGHTS.get(col, 1.0)
                mat *= np.float32(weight)
                for i, tid in enumerate(tids):
                    if tid not in id_set:
                        continue
                    if col in seen_block_by_id[tid]:
                        continue
                    seen_block_by_id[tid].add(col)
                    blocks_by_id[tid].append(mat[i])
                    valid_by_id[tid] = valid_by_id[tid] or bool(mask[i])

    qwen = _load_qwen_track_cache(qwen_track_cache_dir)
    for tid in track_ids:
        if tid in qwen and "qwen_clean" not in seen_block_by_id[tid]:
            blocks_by_id[tid].append(qwen[tid].astype(np.float32))
            valid_by_id[tid] = True

    dense_dim = max((sum(len(v) for v in blocks) for blocks in blocks_by_id.values()), default=0)
    if dense_dim == 0:
        return {}, {}, 0
    out: dict[str, np.ndarray] = {}
    for tid, blocks in blocks_by_id.items():
        vec = np.concatenate(blocks).astype(np.float32) if blocks else np.zeros(dense_dim, np.float32)
        if vec.size < dense_dim:
            vec = np.pad(vec, (0, dense_dim - vec.size))
        out[tid] = _one_l2(vec)
    return out, valid_by_id, dense_dim


def _load_user_cf(paths: list[str] | None) -> dict[str, np.ndarray]:
    if not paths:
        return {}
    out: dict[str, np.ndarray] = {}
    for path in paths:
        ...
        if not p.exists():
            continue
        df = pl.read_parquet(p, columns=["user_id", "cf-bpr"])
        for uid, vec in zip(df["user_id"].to_list(), df["cf-bpr"].to_list()):
            if vec is None or len(vec) != USER_CF_DIM:
                continue
...
def _load_query_embedding_cache(
    ...
    p = _repo_path(cache_dir)
    if not p.exists():
        raise FileNotFoundError(f"[TwoTowerV2Enhanced] query embedding cache not found: {p}")

    out: dict[tuple[str, int], np.ndarray] = {}
    text_dim: int | None = None
    for split in splits or ["train", "dev", "blind_a"]:
        emb_path = p / f"{split}.npy"
        meta_path = p / f"{split}_meta.parquet"
        if not emb_path.exists() or not meta_path.exists():
            print(
                f"[TwoTowerV2Enhanced] query cache split {split!r} missing "
                f"(npy={emb_path.exists()}, meta={meta_path.exists()})"
            )
            continue
        emb = np.asarray(np.load(emb_path, mmap_mode="r"), dtype=np.float32)
        meta = pl.read_parquet(meta_path, columns=["session_id", "turn_number"])
        if emb.shape[0] != meta.height:
            raise ValueError(
                f"Query cache row mismatch for {split}: "
                f"{emb.shape[0]} embeddings vs {meta.height} metadata rows"
            )
        if text_dim is None:
            text_dim = int(emb.shape[1])
        elif text_dim != int(emb.shape[1]):
            raise ValueError(
                f"Query cache dim mismatch: expected {text_dim}, got {emb.shape[1]} "
                f"for split {split}"
            )
        emb = _row_l2(emb)
        for i, row in enumerate(meta.iter_rows(named=True)):
            out[(row["session_id"], int(row["turn_number"]))] = emb[i]
        print(f"[TwoTowerV2Enhanced] loaded {split} Qwen query embeddings: {emb.shape}")

    if not out:
        raise FileNotFoundError(f"No usable query embeddings found under {p}")
    return out, int(text_dim or TEXT_DIM)
...
class _ItemTower(nn.Module):
    ...
    def forward(self, track_idx, artist_idx, tag_idxs, decade_idx, pop_bin, dur_bin,
                dense_vec=None, dense_mask=None):
...
class _EnhancedItemTower(_ItemTower):
    def __init__(self, D: int, dropout: float,
                 n_tracks: int, n_artists: int, n_tags: int, dense_dim: int):
        super().__init__(D, dropout, n_tracks, n_artists, n_tags)
        self.dense_dim = dense_dim
        self.dense_proj = nn.Sequential(
            nn.Linear(dense_dim, D),
            nn.LayerNorm(D), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(D, D), nn.LayerNorm(D),
        ) if dense_dim > 0 else None
        self.mlp = nn.Sequential(
            nn.Linear(D + 64 + 32 + 16 + 8 + 8 + (D if dense_dim > 0 else 0), 256),
            nn.LayerNorm(256), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(256, D), nn.LayerNorm(D),
        )
    ...
    def forward(self, track_idx, artist_idx, tag_idxs, decade_idx, pop_bin, dur_bin,
        ...
        tag_p = (tag_v * mask).sum(-2) / mask.sum(-2).clamp(min=1)
        parts = [
            self.track_emb(track_idx),
            self.artist_emb(artist_idx),
            tag_p,
            self.decade_emb(decade_idx),
            self.pop_emb(pop_bin),
            self.dur_emb(dur_bin),
        ]
        if self.dense_proj is not None:
            if dense_vec is None:
                dense_vec = torch.zeros(
                    (*track_idx.shape, self.dense_dim),
                    dtype=self.track_emb.weight.dtype,
                    device=track_idx.device,
                )
            dense = self.dense_proj(dense_vec.float())
            if dense_mask is not None:
                dense = dense * dense_mask.float().unsqueeze(-1)
            parts.append(dense)
...
class _EnhancedUserTower(_UserTower):
    def __init__(self, item_tower: _ItemTower, D: int, dropout: float,
                 n_ages: int, n_countries: int, n_genders: int,
                 recency_decay: float = 1.0,
                 user_cf_dim: int = USER_CF_DIM,
                 text_dim: int = TEXT_DIM):
        super().__init__(
            item_tower, D, dropout, n_ages, n_countries, n_genders,
            recency_decay=recency_decay,
            text_dim=text_dim,
        )
        self.user_cf_dim = user_cf_dim
        self.user_cf_proj = nn.Sequential(
            nn.Linear(user_cf_dim, D),
            nn.LayerNorm(D), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(D, D), nn.LayerNorm(D),
        )
        self.mlp = nn.Sequential(
            nn.Linear(D + D + D + 56, 256),
            nn.LayerNorm(256), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(256, D), nn.LayerNorm(D),
        )

    def forward(self, txt_emb, ctx, ctx_mask, u_age, u_ctr, u_gen, u_cf=None, u_cf_mask=None):
        txt   = self.text_proj(txt_emb.float())
        B, L  = ctx["track_idx"].shape
        D     = self.item_tower.D
        flat  = {k: v.reshape(B * L, *v.shape[2:]) for k, v in ctx.items()}
        ...
        demo  = torch.cat([self.age_emb(u_age), self.country_emb(u_ctr), self.gender_emb(u_gen)], dim=-1)
        if u_cf is None:
            u_cf = torch.zeros(B, self.user_cf_dim, dtype=txt.dtype, device=txt.device)
        cf = self.user_cf_proj(u_cf.float())
        if u_cf_mask is not None:
            cf = cf * u_cf_mask.float().unsqueeze(-1)
        return self.mlp(torch.cat([txt, ctx_v, cf, demo], dim=-1))
...
class _TwoTowerModel(nn.Module):
    def __init__(self, D: int, dropout: float,
                 ...
                 text_dim: int = TEXT_DIM):
        super().__init__()
        self.item_tower = _EnhancedItemTower(D, dropout, n_tracks, n_artists, n_tags, dense_dim)
        self.user_tower = _EnhancedUserTower(
            self.item_tower, D, dropout, n_ages, n_countries, n_genders,
            recency_decay=recency_decay,
            user_cf_dim=user_cf_dim,
            text_dim=text_dim,
        )
    ...
    def forward(self, batch: dict) -> torch.Tensor:
        u = F.normalize(self.user_tower(
            ...
            batch["u_age"], batch["u_ctr"], batch["u_gen"],
            batch.get("u_cf"), batch.get("u_cf_mask")), dim=-1)
...
def _make_collate(max_ctx: int, track_features: dict, user_features: dict,
    ...
    dense_dim = int(unk_track.get("dense_dim", 0))
    ...
    def collate(batch: list) -> dict:
        ...
        ctx_dn = np.zeros((B, L, dense_dim), np.float32); ctx_dm = np.zeros((B, L), bool)
        u_age  = np.zeros(B, np.int64); u_ctr = np.zeros(B, np.int64); u_gen = np.zeros(B, np.int64)
        u_cf   = np.zeros((B, USER_CF_DIM), np.float32); u_cf_mk = np.zeros(B, bool)
        ...
        out = {
            "ctx": dict(track_idx=T(ctx_ti), artist_idx=T(ctx_ai), tag_idxs=T(ctx_tg),
                        ...
                        dense_vec=T(ctx_dn), dense_mask=T(ctx_dm)),
            "ctx_mask": T(ctx_mk),
            "pos":      pos_feats,
            "u_age": T(u_age), "u_ctr": T(u_ctr), "u_gen": T(u_gen),
            "u_cf": T(u_cf), "u_cf_mask": T(u_cf_mk),
            "txt_emb": T(txt_emb),
        }
        if n_hard_neg > 0:
...
def _item_tensors_batch(tids: list, track_features: dict, unk_track: dict) -> dict:
    ...
    dense_dim = int(unk_track.get("dense_dim", 0))
    dn = np.zeros((B, dense_dim), np.float32); dm = np.zeros(B, bool)
    for i, tid in enumerate(tids):
        f = track_features.get(tid, unk_track)
...
def _ranking_metric(
    ...
    for i in range(0, len(samples), batch_size):
        ...
        uvecs = _encode_users(
            ...
            unk_track, unk_user, device, text_dim=text_dim,
        )
...
def _encode_users(mdl: _TwoTowerModel, sessions: list, max_ctx: int,
                  ...
                  unk_track: dict, unk_user: dict, device,
                  text_dim: int = TEXT_DIM) -> np.ndarray:
    mdl.eval()
    ...
    ctx_t = dict(track_idx=T(ctx_ti), artist_idx=T(ctx_ai), tag_idxs=T(ctx_tg),
                 ...
                 dense_vec=T(ctx_dn), dense_mask=T(ctx_dm))
    u = mdl.user_tower(
        T(txt_emb), ctx_t, T(ctx_mk), T(u_age), T(u_ctr), T(u_gen),
        T(u_cf), T(u_cf_mk),
    )
...
class TwoTowerV2EnhancedRecommender(UserRecommender):
    """Two-Tower v2 enhanced with normalized multimodal item features.

    The extra dense item vector is built from selected organizer modalities and,
    optionally, from a freshly generated Qwen track cache. Each modality is
    row-normalized before weighting and concatenation, then the final vector is
    normalized again. User CF-BPR is loaded with an explicit mask, so cold users
    do not look like a real zero-preference vector.
    """

    RECOMMENDER_NAME = "TwoTowerV2Enhanced"
    ...
    def __init__(
        ...
        track_embedding_paths: list[str] | None = None,
        qwen_track_cache_dir: str | None = None,
        imputed_metadata_embedding_path: str | None = None,
        track_modalities: list[str] | None = None,
        use_item_cf: bool = False,
        item_cf_weight: float = 0.15,
        user_cf_paths: list[str] | None = None,
        use_user_cf: bool = True,
        query_embedding_dir: str | None = None,
        ...
        **kwargs,
        ...
        self.track_embedding_paths = track_embedding_paths
        self.qwen_track_cache_dir = qwen_track_cache_dir
        self.imputed_metadata_embedding_path = imputed_metadata_embedding_path
        self.track_modalities = track_modalities
        self.use_item_cf = use_item_cf
        self.item_cf_weight = item_cf_weight
        self.user_cf_paths = user_cf_paths
        self.use_user_cf = use_user_cf
        self.query_embedding_dir = query_embedding_dir
        self.query_cache_splits = query_cache_splits or ["train", "dev", "blind_a"]
    ...
    def fit(
        ...
        **kwargs,
        ...
        # 3. Query/text embeddings. Prefer precomputed Qwen caches when configured.
        ...
        **kwargs,
        ...
        for i in range(0, len(sessions), BS):
            ...
            uvecs = _encode_users(
                ...
                self._track_features, self._user_features, self._text_embeds,
                self._unk_track, self._unk_user, self._device, text_dim=self._text_dim,
            )
    ...
    def _get_model_state(self) -> dict:
        ...
        st.update({
            ...
            "track_embedding_paths": self.track_embedding_paths,
            "qwen_track_cache_dir": self.qwen_track_cache_dir,
            "imputed_metadata_embedding_path": self.imputed_metadata_embedding_path,
            "track_modalities": self.track_modalities,
            "use_item_cf": self.use_item_cf,
            "item_cf_weight": self.item_cf_weight,
            "user_cf_paths": self.user_cf_paths,
            "use_user_cf": self.use_user_cf,
            "query_embedding_dir": self.query_embedding_dir,
            "query_cache_splits": self.query_cache_splits,
            "_dense_dim": self._dense_dim,
            "_text_dim": self._text_dim,
    ...
    def _set_model_state(self, state: dict) -> None:
        ...
        for k in ("D", "lr", "wd", "batch_size", "max_ctx", "dropout",
                  ...
                  "user_meta_path", "track_embedding_paths", "qwen_track_cache_dir",
                  "imputed_metadata_embedding_path", "track_modalities",
                  "use_item_cf", "item_cf_weight", "user_cf_paths", "use_user_cf",
                  "query_embedding_dir", "query_cache_splits"):
            setattr(self, k, state.get(k, getattr(self, k, None)))
        self._dense_dim      = state.get("_dense_dim", 0)
        self._text_dim       = state.get("_text_dim", TEXT_DIM)
    ...
    def _feature_cache_key(self) -> tuple:
        return (
            str(self.raw_train_path),
            str(self.raw_test_path),
            tuple(self.track_embedding_paths or []),
            str(self.qwen_track_cache_dir),
            str(self.imputed_metadata_embedding_path),
            tuple(self.track_modalities or DEFAULT_TRACK_MODALITIES),
            bool(self.use_item_cf),
            float(self.item_cf_weight),
            tuple(self.user_cf_paths or []),
            bool(self.use_user_cf),
        )
    ...
    def _build_features(self, track_meta: pl.DataFrame,
        ...
        if self.raw_train_path and self.raw_test_path:
            ...
            if _ck in _TT_PRECOMPUTE_CACHE and "track_features" in _TT_PRECOMPUTE_CACHE[_ck]:
                ...
                self._dense_dim      = _c.get("dense_dim", 0)
        ...
        dense_by_tid, dense_mask_by_tid, dense_dim = _load_dense_track_vectors(
            track_ids=track_meta["track_id"].to_list(),
            embedding_paths=self.track_embedding_paths,
            qwen_track_cache_dir=self.qwen_track_cache_dir,
            imputed_metadata_embedding_path=self.imputed_metadata_embedding_path,
            track_modalities=self.track_modalities,
            use_item_cf=self.use_item_cf,
        ...
        zero_dense = np.zeros(dense_dim, np.float32)
        ...
        for r in track_meta.iter_rows(named=True):
            ...
            tf[tid] = {
                ...
                "dense_vec":   dense_by_tid.get(tid, zero_dense),
                "dense_mask":  dense_mask_by_tid.get(tid, False),
                "dense_dim":   dense_dim,
            }
        ...
        self._unk_track = {
            ...
            "dense_vec": zero_dense, "dense_mask": False, "dense_dim": dense_dim,
        }
        ...
        if "user_id" in raw_source.columns and self.user_meta_path:
            ...
            user_cf = _load_user_cf(self.user_cf_paths) if self.use_user_cf else {}
            for r in user_meta.iter_rows(named=True):
                cf_vec = user_cf.get(r["user_id"])
                uf[r["user_id"]] = {
                    ...
                    "cf_vec":      cf_vec if cf_vec is not None else np.zeros(USER_CF_DIM, np.float32),
                    "cf_mask":     cf_vec is not None,
                }
        self._user_features = uf
        self._unk_user = {
            "age_idx": UNK, "country_idx": UNK, "gender_idx": UNK,
            "cf_vec": np.zeros(USER_CF_DIM, np.float32), "cf_mask": False,
        }
        ...
        print(f"[{self.RECOMMENDER_NAME}] tracks={len(track_vocab)}  "
              f"artists={len(artist_vocab)}  tags={len(tag_vocab)}  dense_dim={dense_dim}")
    ...
    def _compute_text_embeds(self, raw_df: pl.DataFrame,
        ...
        if self.query_embedding_dir:
            cache_key = (
                str(self.raw_train_path),
                str(self.raw_test_path),
                "query_cache",
                str(self.query_embedding_dir),
                tuple(self.query_cache_splits or []),
            )
            if cache_key in _TT_PRECOMPUTE_CACHE and "text_embeds" in _TT_PRECOMPUTE_CACHE[cache_key]:
                self._text_embeds.update(_TT_PRECOMPUTE_CACHE[cache_key]["text_embeds"])
                self._text_dim = _TT_PRECOMPUTE_CACHE[cache_key]["text_dim"]
                print(
                    f"[{self.RECOMMENDER_NAME}] Qwen text_embeds restored from cache "
                    f"({len(self._text_embeds):,} entries, dim={self._text_dim})"
                )
                return

            embeds, text_dim = _load_query_embedding_cache(
                self.query_embedding_dir,
                self.query_cache_splits,
            )
            self._text_embeds.update(embeds)
            self._text_dim = text_dim
            print(
                f"[{self.RECOMMENDER_NAME}] using precomputed Qwen query embeddings: "
                f"{len(self._text_embeds):,} entries, dim={self._text_dim}"
            )
            _TT_PRECOMPUTE_CACHE.setdefault(cache_key, {})["text_embeds"] = dict(self._text_embeds)
            _TT_PRECOMPUTE_CACHE[cache_key]["text_dim"] = self._text_dim
            return
        ...
        if not incremental and self.raw_train_path and self.raw_test_path:
            ...
            if _ck in _TT_PRECOMPUTE_CACHE and "text_embeds" in _TT_PRECOMPUTE_CACHE[_ck]:
                ...
                self._text_dim = _TT_PRECOMPUTE_CACHE[_ck].get("text_dim", TEXT_DIM)
        ...
        for row in raw_df.iter_rows(named=True):
            ...
            for c in convs:
                if c["role"] == "music":
                    ...
                    if incremental and (sid, tn) in self._text_embeds:
                        continue
                    lookup[(sid, tn)] = goal + " [SEP] " + user_by_turn.get(tn, "")
            ...
            if has_unanswered_music_context or has_cold_start_unanswered:
                ...
                if not incremental or (sid, tn) not in self._text_embeds:
                    ...
                    lookup[(sid, tn)] = goal + " [SEP] " + last_user
        ...
        self._text_dim = int(embs.shape[1])
        ...
        if not incremental and self.raw_train_path and self.raw_test_path:
            ...
            _TT_PRECOMPUTE_CACHE.setdefault(_ck, {})["text_dim"] = self._text_dim
