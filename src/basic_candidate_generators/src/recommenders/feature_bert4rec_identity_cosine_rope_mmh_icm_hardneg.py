"""mmh_icm + hard negative mining as an auxiliary InfoNCE loss.

The full warm-softmax MLM loss treats all 25K warm items equally; the gradient
is dominated by uniform "easy" negatives. We add an explicit auxiliary term
that pushes the model's score for FEATURE-SIMILAR negatives below the score
for the target, sharpening the decision boundary where it matters most.

  total_loss = MLM_loss + alpha * InfoNCE(target | hard_negs)

Hard negatives are precomputed once at fit time as the top-K nearest warm
items per warm item by RAW-FEATURE cosine similarity (using the same
per-modality-L2 feature stack the encoder ingests).

Caveat: 25K × 25K cosine is a 2.5GB float32 matrix; we batch the lookup.
"""
...
def _build_hard_negatives(
    warm_features_l2: np.ndarray,   # (n_warm, total_dim), already L2-normed per row
    k: int,
    batch: int = 1024,
) -> np.ndarray:
    """Returns (n_warm, k) int32 of top-k nearest warm items per warm item
    by cosine similarity, excluding the item itself."""
    ...
    for start in tqdm(range(0, n_warm, batch), desc="hard_negs", file=sys.stdout):
        ...
        sims = warm_features_l2[start:end] @ warm_features_l2.T   # (b, n_warm)
        # Mask self
        for i in range(end - start):
            sims[i, start + i] = -np.inf
        # Top-k (descending). argpartition gives unordered top-k → re-sort for stability.
...
class FeatureBert4RecIdentityCosineRoPEMMHICMHardNegRecommender(FeatureBert4RecIdentityCosineRoPEMMHICMRecommender):
    """mmh_icm + hard-negative mining InfoNCE auxiliary."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # EMA: 0.0 disables. Typical good values: 0.999 / 0.9995. ema_start_epoch
        # is 1-indexed and controls when EMA updates start (skip first N-1 epochs
        # of warmup so the shadow doesn't get pulled by very high-LR steps).
        self.ema_decay = float(ema_decay)
        self.ema_start_epoch = int(ema_start_epoch)

    def _get_hardneg_weight(self, epoch: int) -> float:
        """Hook for curriculum variants. Default: constant weight.
        Subclasses can override to ramp/decay across epochs.
        `epoch` is 1-indexed (matches the outer loop)."""
    ...
    def _make_dataset(
        ...
        """Factory hook for the train/val Dataset. Default: the standard
        cloze-task dataset for both splits. Subclasses can override to swap in
        an augmented dataset (e.g. random-threshold prefix-cut) on the TRAIN
        side only, keeping val unaugmented and comparable."""
        return _FeatureBert4RecDataset(sequences, n_warm, max_seq_len, mask_prob)

    def _fit_model(self, urm: csr_matrix) -> None:
        # Replay the parent's _fit_model but with an aux InfoNCE on hard negatives.
        # Most of this mirrors the mmh _fit_model; we only differ in the training step.
        ...
        # Pre-compute hard negatives: per-modality L2-normed concat → cosine top-K.
        # This matches the encoder's input geometry.
        ...
        # Re-L2 the full concat so cosine works as expected.
        ...
        hard_neg_tensor = torch.from_numpy(hard_neg_idx).to(self.device_)   # (n_warm, K)
        ...
        # EMA shadow: kept on-GPU as a state_dict-shaped dict for fast updates.
        # Only parameters (not buffers like running mean / RoPE caches) are EMA-ed;
        # buffers are copied verbatim each val pass from the live model.
        ...
        for epoch in epoch_bar:
            ...
            for masked_seq, labels in tqdm(train_loader, desc=f"  ep {epoch:3d}", leave=False,
                ...
                # Hard-negative InfoNCE. For each masked (position, target_warm_local):
                #   anchor logit = logits[pos, target] (already in `logits`)
                #   negs logits  = logits[pos, hard_neg_idx[target]]
                # InfoNCE pulls anchor above its K-nearest competitors.
                with torch.no_grad():
                ...
                if tgt_pos.numel() > 0:
                    tgt_idx = flat_labels[tgt_pos]                       # (M,) warm-local
                    anchor = flat_logits[tgt_pos, tgt_idx].unsqueeze(1)  # (M, 1)
                    hn = hard_neg_tensor[tgt_idx]                        # (M, K)
                    neg_logits = flat_logits[tgt_pos.unsqueeze(1).expand_as(hn), hn]  # (M, K)
            ...
            # EMA val: temporarily swap params, eval, swap back. Only after the
        ...
        if use_ema:
            ...
            print(f"[{self.RECOMMENDER_NAME}] restored {tag} (val={best_val:.4f}, epoch={best_epoch})")
