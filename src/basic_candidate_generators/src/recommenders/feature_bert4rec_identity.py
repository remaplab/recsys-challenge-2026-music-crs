"""Feature-based BERT4Rec with PCA-initialized linear item encoder.

Variant of FeatureBert4RecRecommender. Replaces the 2-layer MLP item_encoder
(1664 → 768 → 256 with GELU+dropout) with a SINGLE linear projection
(1664 → 256) initialized to the top-k principal components of the warm
feature matrix.

Motivation:
  - The MLP starts from random init → has to learn the right projection from
    scratch with limited data.
  - PCA gives the OPTIMAL linear projection that preserves variance.
  - Starting there means the encoder begins with a meaningful representation
    and only needs to fine-tune small deviations.
  - Fewer parameters than the 2-layer MLP → less overfitting risk on a small
    dataset.

The PCA components are computed in _fit_model right after the warm feature
matrix is built, and copied into the encoder's weight/bias before the
training loop starts.

Inherits dataset, recommend, _score_session_sequence, save/load semantics
from the parent. Only the model class and fit-time initialization differ.
"""
...
# ---------------------------------------------------------------------------
# Model: single-linear item encoder (PCA-init done from the recommender)
# ---------------------------------------------------------------------------

class _FeatureBert4RecIdentityModel(_FeatureBert4RecModel):
    """Parent model with item_encoder replaced by a single Linear projection.

    This linear can be PCA-initialized from the outside (recommender fit()).
    The transformer body, special embeddings, positional embeddings, and
    output_norm are inherited unchanged.
    """
    ...
    def __init__(
        ...
        # Override the MLP encoder with a single linear projection
...
class FeatureBert4RecIdentityRecommender(FeatureBert4RecRecommender):
    """Single-linear item_encoder with PCA initialization.

    Same training loop and inference path as parent — only the model class
    and the pre-training PCA init step differ.
    """

    RECOMMENDER_NAME = "FeatureBert4RecIdentity"

    def _pca_init_encoder(self, feature_matrix: np.ndarray) -> None:
        """Set item_encoder weights so its output equals PCA(features).

        For PCA with components W (shape: hidden, feature_dim) and mean μ:
            output = (x - μ) @ W.T = x @ W.T - μ @ W.T
        Linear layer Wx + b applies W (out_features, in_features) to x:
            output = x @ W.T + b
        So we set:
            W (linear) = W (PCA)  → (hidden, feature_dim)
            b          = -μ @ W.T  → (hidden,)
        """
        ...
        W = torch.from_numpy(pca.components_.astype(np.float32))         # (hidden, feature_dim)
        mean = torch.from_numpy(pca.mean_.astype(np.float32))            # (feature_dim,)
        b = -(mean @ W.T)                                                # (hidden,)
    ...
    def _fit_model(self, urm: csr_matrix) -> None:
        ...
        # New model class with single-linear item encoder
        ...
        # PCA-initialize the linear projection BEFORE moving to device
    ...
    # ------------------------------------------------------------------
    # save / load — same fields as parent, but model class differs
    # ------------------------------------------------------------------
