"""Identity (PCA-init single-linear encoder) + cosine similarity + learned tau.

Combines the two best ideas:
  - Identity: single Linear item_encoder, PCA-initialized
    (winner so far on r@200 / r@500)
  - Cosine + tau: magnitude-invariant scoring with a learnable temperature
    (removes the magnitude advantage of well-trained warm items over cold ones)

Inherits everything from FeatureBert4RecIdentityRecommender; only overrides:
  - the model class (single linear + cosine output + log_tau)
  - the model factory inside _fit_model
  - the inference scoring (cosine instead of dot product)
  - the load path

The PCA initialization is reused as-is — direction of components is what
PCA captures, magnitude is now irrelevant because we normalize before scoring.
"""
...
# ---------------------------------------------------------------------------
# Model: single-linear item encoder (PCA-init) + cosine scoring with tau
# ---------------------------------------------------------------------------

class _FeatureBert4RecIdentityCosineModel(_FeatureBert4RecIdentityModel):
    """Identity model with cosine output scoring scaled by 1/tau.

    Input sequence embeddings stay unnormalized so the transformer keeps
    magnitude information internally; only the final dot product against
    warm items becomes cosine.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return (out_n @ warm_n.T) / self.tau  # (B, L, n_warm)
...
class FeatureBert4RecIdentityCosineRecommender(FeatureBert4RecIdentityRecommender):
    ...
    # ------------------------------------------------------------------
    # Inference: cosine scoring (matches forward())
    # ------------------------------------------------------------------
