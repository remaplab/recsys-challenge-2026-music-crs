"""Cross-validation hyperparameter tuning for Candidate Generators (One-Shot / Turn 1).

Each Optuna trial trains on the cg_train split and evaluates EXCLUSIVELY on Turn 1
of the cg_val split to optimize Cold Start performance.
Primary objective: ndcg@20 (mean across folds).

Usage:
    cd src/basic_candidate_generators
    uv run python -m launchers_crossvalidation.tune_crossvalidation \
        --model two_tower [--n_trials 30] [--n_jobs 1] \
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="CV hyperparameter tuning for a single CG model (One-Shot).")
...
def _run_one_mode(
    ...
    fixed_params   = mcfg.get("fixed_params") or {}
    ...
    top_k      = int(eval_cfg.get("top_k", 20)) # Ridotto il top_k necessario per ottimizzare su ndcg@20
    
    recall_ks  = [int(k) for k in eval_cfg.get("recall_ks", RECALL_KS)]
    ndcg_ks    = [int(k) for k in eval_cfg.get("ndcg_ks", NDCG_KS)]
    
    # Assicuriamoci che 20 sia nelle metriche da calcolare
    if 20 not in ndcg_ks:
        ndcg_ks.append(20)
    ...
    folder_key  = f"{model_name}_{urm_mode}_oneshot" # Aggiunto suffisso per non sovrascrivere studi vecchi
    ...
    print(f"\n{'='*60}")
    print(f"[cv] model:      {model_name} ({class_name}) - ONE SHOT MODE")
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        try:
            for fold in range(n_folds):
                ...
                # --- FILTRO ONE-SHOT (TURN 1 ONLY) ---
                # Valutiamo esclusivamente il primo turno
                eval_df = eval_df.filter(pl.col("turn_number") == 1)
                
                # Se desideri che anche l'addestramento avvenga ESCLUSIVAMENTE sul Turno 1
                # (per massimizzare la specializzazione a discapito della mole di dati),
                # decommenta la riga sottostante:
                # train_df = train_df.filter(pl.col("turn_number") == 1)
                # -------------------------------------
                ...
                fold_ndcg20.append(metrics["ndcg@20"])
                all_fold_metrics.append(metrics)

                mean_so_far = sum(fold_ndcg20) / len(fold_ndcg20)
                elapsed     = time.time() - t0
                print(f"  [trial {trial.number}] fold {fold}: ndcg@20={metrics['ndcg@20']:.4f}"
    ...
    if remaining == 0:
        ...
        def _callback(_study: optuna.Study, t: optuna.trial.FrozenTrial) -> None:
            ...
            print(f"[trial {t.number:>4d}] ndcg@20={t.value:.6f}  {ps}\n")
    ...
    if study.best_trial is not None:
        print(f"\n[cv] best ndcg@20: {study.best_value:.6f}  (trial #{study.best_trial.number})")
